...
from geodlkit.core import Preprocess, Train, Postprocess
from geodlkit.utils import merge_coco, Config, create_project_structure
from geodlkit.model import maskrcnn_rgb, maskrcnn_rgb_highres
from geodlkit._version import __version__

def create_parser():
    synopsis = 'This is a python interface for geodlkit program'
    name = 'geodlkit'
    parser = argparse.ArgumentParser(name, description=synopsis, add_help=True)
    parser.add_argument("-v", "--version", action='version', version=f'geodlkit {__version__}')
    parser.add_argument('-g', '--get_config', metavar='PATH', help='Get the default config file to given path')
    parser.add_argument('-e', '--example_data', metavar='PATH', help='Download example dataset to ~/geodlkit_example_data')
...
def download_example_data(download_path=None):
    ...
    if download_path is None:
        dest_folder = Path.home() / 'geodlkit_example_data'
        dest_folder.mkdir(exist_ok=True)
    else: 
        dest_folder = Path(download_path) / 'geodlkit_example_data'
        dest_folder.mkdir(exist_ok=True)
